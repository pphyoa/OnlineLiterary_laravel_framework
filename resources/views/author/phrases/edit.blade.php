    ...
    <form action="" method="post" enctype="multipart/form-data">         
      ...
      <div class="row">
        <div class="col-md-6">
          ...
          <input type="text" class="form-control" id="title" name="title" value="{{$phrases->title}}">
        </div>
        <div class="col-md-6">
          <label for="author">ဘာသာပြန်ဆရာ</label>
          <input type="text" class="form-control" id="author" name="user_id" value="{{$user->id}}">
        </div>
        <div class="col-md-6">
            <label for="author">စာစဉ်နံပါတ်</label>
            <input type="text" class="form-control" id="author" name="bulk" value="{{$phrases->bulk}}">
          </div>
        <div class="col-md-6">
            <label for="image">ဓာတ်ပုံ</label>
            <input type="file" id="image" name="img" class="form-control" value="{{$phrases->img}}">
          </div>
      </div>
      <div class="form-group">
        <label for="vide">စာမြည်း</label>
        <textarea class="form-control" name="content" id="vide" cols="20" rows="5">{{$phrases->content}}</textarea>
      </div>
